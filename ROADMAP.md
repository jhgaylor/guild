...
- _Nothing in flight._ **G5 complete** (all 4 slices merged + deployed + live-verified 2026-05-29). See Gated → G5. Next: frame G6.
...
- **G6 (not yet framed).** Carry-overs from G4/G5: Linear `:executing→In Progress` transition (helper exists, unused); bidirectional adapter sync (Linear/Slack → Guild events); Slack interactive buttons / Events API (G5 did slash commands only); operator digest (Slack summary of shipped/in-flight/stuck); OTP 28.1+ base-image bump (regex-recompile perf warning); Pass D efficiency (mark conv artifact `:terminated` to skip re-`get_status` each cycle); `state_entered_at` column for precise stuck-detection (G5 uses `updated_at` proxy); thread-timeline polish (interleaved view + `context_snapshot`-nil indicator). Frame into a G6 scope doc before dispatching.
