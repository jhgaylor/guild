...
- **G5 (not yet framed).** Carry-over follow-ups surfaced during G4: **(reconcile robustness, higher priority)** a worker conv that does not go `:idle` after opening its PR leaves the thread stuck in `executing` — reconcile Pass A requires `:idle`; observed live 2026-05-29 (conv `84f004f3` for issue #34 still `:running` 15+ min after opening PR #35). Also: reconcile only matches **open** PRs, so a PR opened+merged between ticks never advances the thread (`executing` forever). Investigate worker-conv idle lifecycle + make reconcile also reconcile against merged PRs. Plus: wire the optional Linear `:executing → In Progress` transition (helper exists, unused); `threads.owner` release semantics (cleared on terminal state / worker restart — currently set-and-leave); Oban Web UI for operator job visibility; bidirectional adapter sync (Linear/Slack → Guild events); bump base image to OTP 28.1+ (regex-recompile perf warning). Frame these into a G5 scope doc before dispatching.
